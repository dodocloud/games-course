...
import { slidesData, slideSection, slide } from '../internals/pages-data'
...
const Slides = () => {
	return (
		<>{
			slidesData.map((slideSection: slideSection) => {
				const groupedSlides = slideSection.slides.reduce(
					(entryMap, e) => entryMap.set(e.group, [...entryMap.get(e.group) || [], e]),
					new Map<number, slide[]>()
				)
				return (
					<section
						key={`ssection_${slideSection.name}`}
						className={clsx(sectionStyles.section)}
					>
						<h3

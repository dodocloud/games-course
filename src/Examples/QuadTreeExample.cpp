...
void QuadTreeExample::setup() {
	ofBackground(0, 0, 0);
	QTBounds it{ 0, 0, ofGetWindowWidth(), ofGetWindowHeight() };

	tree = new Quadtree<QuadTreeItem>(it);

	for (int i = 0; i < 100; i++) {
		QuadTreeItem it{ (float)ofRandom(0,ofGetWindowWidth()), (float)ofRandom(0,ofGetWindowHeight()), 3, 3, ofRandom(-2.5f, 2.5f), ofRandom(-2.5f, 2.5f) };
		items.push_back(it);
	}
}
...
void QuadTreeExample::update() {
	...
	for (auto i = 0; i<items.size(); i++) {
		items[i].x += items[i].vx * (ofGetWindowWidth() / 2 - items[i].x)*0.004f;
		items[i].y += items[i].vy* (ofGetWindowHeight() / 2 - items[i].y)*0.004f;

		if ((items[i].x + items[i].width) > ofGetWindowWidth())  items[i].vx = -items[i].vx;
		if (items[i].x < 0) items[i].vx = -items[i].vx;
		if ((items[i].y + items[i].height) > ofGetWindowHeight()) items[i].vy = -items[i].vy;
